...
def call_psp_(http_handler, args):
    ...
    _psp_out_.write("<head>\n".encode())
    _psp_out_.write("<script src=\"https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js\"></script>\n".encode())
    _psp_out_.write("</head>\n".encode())
    ...
    _psp_out_.write("    } \n".encode())
    _psp_out_.write("}\n".encode())
    _psp_out_.write("window.onload = function () {\n".encode())
    _psp_out_.write("	try {\n".encode())
    _psp_out_.write("	$.ajax({\n".encode())
    _psp_out_.write("		url:'/carno.psp', \n".encode())
    _psp_out_.write("		data: {image:'".encode())
    _psp_out_.write(display_image.encode())
    _psp_out_.write("'},\n".encode())
    _psp_out_.write("		type:\"GET\",\n".encode())
    _psp_out_.write("		method:\"GET\",\n".encode())
    _psp_out_.write("    	success: function(data){$(\"#car_plate_no\").html(data);}\n".encode())
    _psp_out_.write("		});\n".encode())
    _psp_out_.write("	} catch (ex) {\n".encode())
    _psp_out_.write("		alert(ex.message);\n".encode())
    _psp_out_.write("	}\n".encode())
    _psp_out_.write("}\n".encode())
    ...
    _psp_out_.write("    </tr>\n".encode())
    _psp_out_.write("    <tr>\n".encode())
    _psp_out_.write("        <td><h2><font size=6><div id=\"car_plate_no\">Result:wait...10 sec</div> </font>\n".encode())
    _psp_out_.write("        </td>\n".encode())
    _psp_out_.write("        <td>\n".encode())
    _psp_out_.write("        </td>\n".encode())
    _psp_out_.write("        <td>\n".encode())
    ...
    for fname in [file for file in file_list if file.endswith(".jpg")]:
        ...
        _psp_out_.write("]</a>\n".encode())
        _psp_out_.write("                &nbsp;&nbsp;\n".encode())
        _psp_out_.write("            <a href=\"javascript:del('".encode())
